...
@login_required(login_url='home.html')
def confir(request, pk):
    return render(request, "confir.html", {'currentOrder': pk})
...
def payment(request, pk):
    current = request.user
    phoneAuth = request.POST.get('phoneCardAuth')  # Use get to avoid MultiValueDictKeyError
    emailAuth = request.POST.get('emailCardAuth')  # Use get to avoid MultiValueDictKeyError
    ...
    pricePennies = int(databaseOrder.price * 100)
    ...
    if request.method == 'POST' and phoneAuth == current.customer.phone and emailAuth == current.customer.email:
        ...
        context = {'currentOrder': pk}
        return render(request, 'success', context)
    else:
        return redirect('confir', pk=pk)
@login_required(login_url='home.html')
